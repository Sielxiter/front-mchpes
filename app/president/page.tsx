...
import {
  ...
  type PresidentCommissionMeta,
} from "@/lib/api"

type Meta = { page: number; per_page: number; total: number; last_page: number } & PresidentCommissionMeta

export default function Page() {
  const [fetching, setFetching] = useState(true)
  const [items, setItems] = useState<PresidentDossier[]>([])
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof PresidentApiError) {
  ...
  const title = useMemo(() => {
    ...
    return spec ? `Président — ${spec}` : "Président"
  }, [meta])
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            PV, note finale, validation des résultats.
          </p>
        </div>
              ...
              <CardHeader className="space-y-1">
                ...
                <CardAction>
                  <Button asChild size="sm" variant="outline">
                    <Link href={`/president/dossiers/${c.id}`}>Ouvrir</Link>
                  </Button>
                </CardAction>
                <div className="text-sm text-muted-foreground">
